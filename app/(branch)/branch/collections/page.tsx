...
export default function BranchCollectionsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      if (!canQuery) return;
      setLoading(true);
      const data = await SalesService.listOrders({ branchId: branchId as string });
      setOrders(data);
      setLoading(false);
    };
    load();
      ...
      <div className="p-8 w-full">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600">Manage collections for your branch invoices</p>
          </div>
        </div>
                  ...
                  {orders.map((o) => (
                    <tr key={o.id}>
                      ...
                      <td className="px-4 py-2 text-right">
                        <button
                          onClick={() => router.push(`/branch/sales/${o.id}/edit`)}
