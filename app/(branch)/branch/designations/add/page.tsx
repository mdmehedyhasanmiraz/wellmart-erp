...
export default function BranchAddDesignationPage() {
  ...
  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setForm((prev) => ({
      ...prev,
      name,
      code: DesignationService.generateCode(name),
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const created = await DesignationService.create(form);
      if (created) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 w-full">
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Add Designation</h1>
        <p className="mt-2 text-gray-600">Create a new designation for the organization.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            {saving ? 'Saving…' : 'Create Designation'}
          </button>
        </div>
      </form>
    </div>
