  ...
  useEffect(() => {
    const loadSuppliers = async () => {
      try {
        ...
        const suppliersData = await SupplierService.getSuppliersByBranch(userProfile.branch_id);
      ...
      <div className="p-8 w-full">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600">Manage suppliers and vendors for {userProfile?.branch_name || 'your branch'}</p>
          </div>
          <Link
            href="/branch/suppliers/new"
                  ...
                  <tr key={s.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center space-x-2">
                        <button onClick={() => router.push(`/branch/suppliers/${s.id}`)} className="text-purple-600 hover:text-purple-900">View</button>
                        <button onClick={() => router.push(`/branch/suppliers/${s.id}/edit`)} className="text-blue-600 hover:text-blue-900">Edit</button>
                      </div>
                    </td>
                  </tr>
                ))}
