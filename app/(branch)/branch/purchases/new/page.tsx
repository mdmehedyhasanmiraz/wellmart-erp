...
export default function BranchNewPurchasePage() {
  ...
  const handleProductChange = async (idx: number, productId: string) => {
    ...
    if (userProfile?.branch_id && productId && !productTotals[productId]) {
      try {
        const stocks = await InventoryService.getBatchStocksByBranch(productId, userProfile.branch_id);
  ...
  const ensureTotalsFor = async (ids: string[]) => {
    ...
    if (!userProfile?.branch_id || missing.length === 0) return;
    try {
      const results = await Promise.all(missing.map(async (id) => {
        const stocks = await InventoryService.getBatchStocksByBranch(id, userProfile.branch_id!);
        const total = (stocks || []).reduce((s, r) => s + (r.quantity || 0), 0);
